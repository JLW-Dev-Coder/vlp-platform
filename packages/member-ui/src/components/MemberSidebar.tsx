...
export function MemberSidebar({ config, onSignOut, drawerOpen = false, onCloseDrawer }: MemberSidebarProps) {
  ...
  useEffect(() => {
    if (!drawerOpen || !onCloseDrawer) return
  ...
  function renderSidebarBody({
    ...
    return (
      <>
        {/* Logo */}
        <div className={`flex items-center ${isCollapsed ? 'justify-center px-2' : 'gap-3 px-6'} py-5`}>
          <span
              ...
              <div className="space-y-0.5">
                {section.items.map((item) => {
                  ...
                  const linkClassName = `flex flex-1 items-center rounded-lg py-2 text-sm font-medium transition ${
                    ...
                    active
                      ? `${isCollapsed ? '' : 'border-l-2'} text-white`
                      : `${isCollapsed ? '' : 'border-l-2 border-transparent'} text-white/60 hover:bg-white/[0.04] hover:text-white`
                  }`
                  const linkStyle = active
                    ? {
                        borderColor: isCollapsed ? undefined : config.brandColor,
  ...
  return (
    <>
      {/* Desktop sidebar — hidden at < md */}
      <aside
        className={`hidden md:flex shrink-0 flex-col border-r border-white/[0.08] bg-[var(--member-bg)] transition-[width] duration-200 ${
          collapsed ? 'w-[68px]' : 'w-[240px]'
        }`}
      >
        {renderSidebarBody({ isCollapsed: collapsed, showCollapseToggle: true })}
      </aside>

      {/* Mobile drawer — renders at < md only */}
      <div className="md:hidden">
        {/* Backdrop */}
        <div
          className={`fixed inset-0 z-overlay bg-black/50 transition-opacity duration-base motion-reduce:transition-none ${
        ...
        {/* Drawer panel */}
        <aside
          className={`fixed inset-y-0 left-0 z-modal flex w-[85vw] max-w-[320px] flex-col overflow-y-auto border-r border-white/[0.08] bg-[var(--member-bg,var(--surface-bg))] transition-transform duration-base motion-reduce:transition-none ${
        ...
        >
          ...
          <button
            type="button"
            onClick={onCloseDrawer}
            className="absolute right-3 top-3 z-10 rounded-md p-2 text-white/40 transition hover:bg-white/[0.04] hover:text-white"
