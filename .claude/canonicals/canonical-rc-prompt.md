...
| 2026-05-07 | Anti-pattern documented: "replace entire file with:" without first reading existing content | During the ROLES.md update on 2026-05-06, Principal authored a "replace entire file with:" instruction without first asking RC to dump the existing content. RC executed literally and the existing "Brevity rule (global, all roles)" section was silently dropped. Caught only because RC noticed the line count mismatch and flagged. Going forward: when authoring updates to existing canonical/role/contract files, Principal should either (a) ask RC to paste the current file content first then author a replacement that preserves all existing sections, or (b) use targeted str_replace-style edits that add without nuking. |
