...
14. **TCVLP ESLint `eslint-config-next` module-resolution warning.** Pre-existing, non-blocking warning during TCVLP builds about flat-config module resolution. Does not affect build success. Fix during Phase 3 tooling pass when ESLint configs across all 8 apps are reviewed for consistency.
