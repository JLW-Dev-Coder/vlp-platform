...
10. **Document Tailwind-key-vs-CSS-var distinction for runtime brand tokens.** When a brand token is declared in an app's `tailwind.config.ts` under `theme.extend.colors.brand`, it only generates utility classes (e.g., `text-brand-text-on-primary`). It does NOT automatically create a CSS custom property. Components that read the token at runtime via inline style (e.g., `style={{ color: 'var(--brand-text-on-primary)' }}`) require the corresponding `--brand-*` variable to be declared in the app's `globals.css`. TCVLP surfaced this during A3.3 when the submit-button text-color fix required both the Tailwind key AND a CSS var. Document this dual-layer requirement in canonical-style.md §2 (Brand Tokens) with an explicit example.
11. **Fix false `border-border-default` references.** canonical-style.md §2.3 and canonical-app-components.md §5.5 reference a `border-border-default` class that does not exist in the shared Tailwind preset. The actual usage pattern is either bare `border` (which uses the CSS DEFAULT resolved from the preset) or explicit `border-[var(--border-default)]`. Update both canonical references to reflect the real available class forms.
12. **Convert `focus:` to `focus-visible:` in `FullCalendar.tsx`.** One shared component still uses bare `focus:` classes on non-input interactive elements. Inputs should stay on `focus:` (correct for text fields), but buttons/links/clickable cells should convert to `focus-visible:` to match the a11y pattern established across MarketingHeader, MarketingFooter, and ReviewSubmitPage. Audit each `focus:` occurrence in `packages/member-ui/src/components/FullCalendar.tsx` and convert only the non-input elements.
13. **Extract `apps/tcvlp/components/Card.tsx` to shared package.** Small local card component in TCVLP that matches patterns used in multiple apps. Candidate for lift into `@vlp/member-ui` as a canonical `Card` primitive. Evaluate during Phase 3 shared-component rollout when reviewing what the 6 remaining apps each have as local card-adjacent components.
14. **Fix TCVLP `.claude/CLAUDE.md` AppShell documentation error.** TCVLP's app-level CLAUDE.md claims the dashboard uses the shared AppShell component. Actual implementation uses a bespoke local sidebar. Correct the doc when Phase 3 addresses the authenticated dashboard migration (AppShell canonicalization or the bespoke sidebar being replaced).
15. **TCVLP ESLint `eslint-config-next` module-resolution warning.** Pre-existing, non-blocking warning during TCVLP builds about flat-config module resolution. Does not affect build success. Fix during Phase 3 tooling pass when ESLint configs across all 8 apps are reviewed for consistency.
